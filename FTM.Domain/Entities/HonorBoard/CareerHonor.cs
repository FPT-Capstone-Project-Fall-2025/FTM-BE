using System;
using System.ComponentModel.DataAnnotations;
using FTM.Domain.Entities.FamilyTree;

namespace FTM.Domain.Entities.HonorBoard
{
    /// <summary>
    /// Represents a career achievement or professional honor awarded to a family tree member
    /// </summary>
    public class CareerHonor : BaseEntity
    {
        /// <summary>
        /// Foreign key to the family tree member who earned this honor
        /// </summary>
        [Required]
        public Guid GPMemberId { get; set; }

        /// <summary>
        /// Foreign key to the family tree this honor belongs to
        /// </summary>
        [Required]
        public Guid FamilyTreeId { get; set; }

        /// <summary>
        /// Title or name of the career achievement (e.g., "CEO of Fortune 500 Company", "Outstanding Employee Award")
        /// </summary>
        [Required]
        [MaxLength(300)]
        public string AchievementTitle { get; set; } = null!;

        /// <summary>
        /// Name of the organization or company
        /// </summary>
        [Required]
        [MaxLength(300)]
        public string OrganizationName { get; set; } = null!;

        /// <summary>
        /// Position or role held
        /// </summary>
        [MaxLength(200)]
        public string? Position { get; set; }

        /// <summary>
        /// Year when the achievement or recognition was earned
        /// </summary>
        [Required]
        public int YearOfAchievement { get; set; }

        /// <summary>
        /// Detailed description or notes about the achievement
        /// </summary>
        [MaxLength(2000)]
        public string? Description { get; set; }

        /// <summary>
        /// URL to photo or certificate image
        /// </summary>
        [MaxLength(500)]
        public string? PhotoUrl { get; set; }

        /// <summary>
        /// Whether this honor is currently displayed on the honor board
        /// </summary>
        public bool IsDisplayed { get; set; } = true;

        // Navigation properties
        public virtual FTMember GPMember { get; set; } = null!;
        public virtual Entities.FamilyTree.FamilyTree FamilyTree { get; set; } = null!;
    }
}
